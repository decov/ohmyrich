...
// verificar e definir a configuração de presença global
void config_t::update() {
    // abrir arquivo de configuração
    std::ifstream config_file(CONFIG_PATH);
    
    if (config_file.fail()) {
        printf("arquivo de configuração não localizado\n");
    ...
    // isso é super específico e NÃO é uma análise ini adequada
    // mas funciona, economiza memória e evita dependências massivas
    for (std::string line; std::getline(config_file, line);) {
        // se a linha for um comentário ini (;), espaço em branco, ou um '[', pular
        ...
        if (std::getline(line_stream, key, '=')) {
            ...
            // condições especiais para carimbos de data e hora para evitar valores ruins
...
void config_t::print() {
    ...
    printf("\nPresença Atual (%s) :", this->client_id.c_str());
    printf("\nEstado: %s", this->state.c_str());
    printf("\nDetalhes: %s", this->details.c_str());
    printf("\nImagem Grande: '%s' com toolip, '%s'", this->large_img.key.c_str(),
           this->large_img.text.c_str());
    printf("\nImagem Pequena: '%s' com toolip, '%s'", this->small_img.key.c_str(),
           this->small_img.text.c_str());
    printf("\nHora de Início: %lld", this->start_time);
    printf("\nHora do Final: %lld\n", this->end_time);
